...
func TestNewMandelbrotRGBA(t *testing.T) {
	...
	assert.NotNil(t, img)
	assert.Equal(t, image.Rectangle{Min: image.Point{X: 0, Y: 0}, Max: image.Point{X: config.DefaultConfiguration.Image.CanvasWidth, Y: config.DefaultConfiguration.Image.CanvasHeight}}, img.Bounds())
...
func TestCalculateGrayscalePixelColor(t *testing.T) {
	...
	assert.Equal(t, color.RGBA{51, 51, 51, 0xff}, calculateGrayscalePixelColor(20, 100))
}
